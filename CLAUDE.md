...
| **Boat draft** (RoF2 floats boats client-side — server z AND model-geometry shifts are both re-floated away; boats ride ~4 high) | `src/boat_draft.cpp` (`/rcpboat`, ini `[Boat] Draft`, default 4) | ✅ yes | `make install` (.asi) | The sim writes the waterline every tick into 3 DLL-side nodes off the wrapper actor (`+0x028→z+0x38`, `+0x074→z+0x6C`, `+0x180→z+0x44`; wrapper's own `+0x34/+0x11C` are non-rendering caches) — RE'd live via `/rcpboat blast/dump`. The draft is re-asserted per frame in **BeginScene** (post-sim/pre-draw; new `directx::add_prerender_callback` seam), change-detected per field, X-validated per node. Probes kept: watch/dump/blast/setz/seta. `tools/patch_ship_z.py` (mesh CenterOffset shift) was tried first and REVERTED — the client re-floats from geometry; `freporte_chr.s3d` is stock |
