...
HMODULE find_module(std::string regex_str) {
  ...
  while (Module32Next(hModuleSnap, &me32)) {
    // MODULEENTRY32 resolves to the ANSI form here (mingw, no UNICODE), so
    // szModule is a char array we can read straight into a std::string.
    std::string name = me32.szModule;
